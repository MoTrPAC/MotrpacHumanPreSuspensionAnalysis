...
[MoTrPAC data access procedures](https://motrpac-data.org/project-overview#acute-exercise).
...
- **`exercise_no_controls`** (10 contrasts) — Within-group change from
  ...
  control group changes. These are purely for comparison to describe the effect
  of the controls. These are not presented as a main result in the manuscript. 
...
```{r contrast-barplot, fig.width = 6, fig.height = 3}
...
```{r contrast-names-by-type, results = "asis"}
...
```{r filter-primary-contrasts, eval = FALSE}
...
```{r feature-to-gene-str, eval = FALSE}
...
```{r plot-single-feature-tfeb, fig.width = 5, fig.height = 3}
...
```{r plot-feature-heatmap, eval = FALSE}
...
```{r plot-enrich-heatmap, eval = FALSE}
# Note: filename is required — the heatmap is saved directly to PDF.
...
```{r plot-cluster-enrichment, eval = FALSE}
# Note: filename is required — the heatmap is saved directly to PDF.
...
```{r run-cameraPR-muscle, eval = FALSE}
...
```{r run-cluster-cameraPR, eval = FALSE}
FCM <- run_cmeans()
