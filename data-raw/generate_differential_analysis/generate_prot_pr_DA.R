#' Prepare and run protein ratio proteomics differential analysis inputs
#'
#' This internal helper function prepares protein ratio proteomics (Prot-PR)
#' data for differential analysis using linear mixed models. QC-filtered and
#' batch-corrected normalized values are used directly as model inputs, without
#' additional transformation.
#'
#' Because Prot-PR data contain missing values, features are first filtered using
#' a paired-sample criterion to ensure adequate representation across groups and
#' timepoints. Model fitting is performed using the \code{dream} framework.
#' Users running DREAM-based analyses must use **R version 4.3 or higher** to
#' ensure correct handling of missing data in mixed models.
#'
#' Although both acute and training analyses are supported internally, **only
#' acute exercise Prot-PR results are publicly released** due to limited sample
#' sizes in training cohorts. Certain tissue–ome combinations (e.g., adipose
#' Prot-PR) do not contain training samples and are automatically skipped.
#'
...
#' \enumerate{
#'   \item Load QC-filtered and batch-corrected Prot-PR data
...
#'
#' @note
#' Updated to reflect reduced sample sizes and missingness characteristics in
#' proteomics data. Prot-PR, Prot-PH, and Prot-OL are implemented as separate
#' helpers for historical and assay-specific reasons.
#'
#' @keywords internal proteomics
#' @author christopher jin

.generate_prot_pr_inputs = function(repo_local_dir,
                                    acute_vs_training,
                                    tissue,
                                    parallel = F){
  if(acute_vs_training == "training" & tissue == "adipose"){
    ...
    if (length(ome_data) > 0) {
      ...
      if (acute_vs_training == "acute") metadata = metadata %>% filter(visitcode == 'ADU_BAS')  #filter just to the initial acute bout
