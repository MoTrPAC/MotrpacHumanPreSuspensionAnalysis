...
The package version names the data collection it is built on. In a version such
as 2.0.8, the first two digits are the MoTrPAC data collection (2.0 is c2.0) and
the last digit counts package updates made on top of that collection. A new data
collection starts a new version series.

- **Version 0.2.0** generated the figures in the initial bioRxiv pre-print. It
  predates the c2.0 collection, so install it to reproduce those figures exactly.
- **The 2.0.x series** is built on c2.0 and, together with the other packages
  listed below, is the basis of the forthcoming pre-print.

Over the course of peer review and additional analysis, modifications may
occur. Each release is listed in the GitHub “Releases” section.
...
**On macOS, install [XQuartz](https://www.xquartz.org) before installing this
package**, then log out and back in. Windows and Linux do not need it.

The macOS builds of R link Tcl/Tk and the cairo graphics device against the X11
libraries XQuartz provides. This package imports Mfuzz, which imports `tcltk`,
so without XQuartz the installation fails when R test-loads the package, with
an error like:
...
`plot_enrich_heatmap()` and `plot_cluster_enrichment()` also write PDFs with
`cairo_pdf()`, which needs XQuartz on macOS.

## GitHub-only dependency: MotrpacBicQC

This package imports [MotrpacBicQC](https://github.com/MoTrPAC/MotrpacBicQC)
(>= 2.0.0), which is not on CRAN or Bioconductor. `DESCRIPTION` declares it in
...
If the private package is not installed, it will return a clear error message
with access guidance.
...
By default, load_summary_stats() loads group- and timepoint-level summary statistics for normalized expression data in a nested list structure, organized identically to the differential-analysis datasets. The top level corresponds to tissues, and the second level corresponds to molecular assays.

The research metabolomics platforms arrive as a single `metab` element per tissue, not one element per platform. That table reads `assay = "metab"` and names the platform in its own `platform` column, exactly as `{TISSUE}_METAB_DA` does, so a summary statistic and the differential-analysis row it belongs to agree on what the ome is called. Naming any one platform loads the whole stack. Clinical chemistry (`metab-t-clinical`) is not in it and stays its own object.
...
* Data used in the preparation of this article were obtained from the Molecular Transducers of Physical Activity Consortium (MoTrPAC) Pre-CovidSuspension Data release version c2.0.
